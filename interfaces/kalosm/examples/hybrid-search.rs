...
pub enum HybridSearchError<E> {
    ...
    SemanticSearchError(#[from] DocumentTableSearchError<E>),
...
impl<E> From<EmbeddedIndexedTableError> for HybridSearchError<E> {
    fn from(err: EmbeddedIndexedTableError) -> Self {
        HybridSearchError::SemanticSearchError(DocumentTableSearchError::from(err))
    }
}
...
fn normalize_scores(scores: &[f32]) -> Option<Vec<f32>> {
    if scores.is_empty() {
        return Some(vec![]);
    }
...
fn calculate_rrf_score(rank: usize, k: f32) -> f32 {
...
impl<C: Connection, E, K: Chunker> DocumentTableBuilder<C, E, K> {
    ...
    /// # }
    /// ```
    pub fn with_hybrid_search(self) -> HybridDocumentTableBuilder<C, E, K> {
...
impl<C: Connection, E, K: Chunker> HybridDocumentTableBuilder<C, E, K> {
    ...
    pub async fn build<R>(self) -> Result<HybridDocumentTable<C, R, E, K>, HybridSearchError<E::Error>>
    where
        E: Embedder,
...
impl<C: Connection, R, E: Embedder, K: Chunker> HybridDocumentTable<C, R, E, K> {
    ...
    pub async fn insert(&self, value: R) -> Result<RecordIdKey, HybridSearchError<E::Error>>
    where
    ...
    pub async fn rebuild_search_index(&self) -> Result<(), HybridSearchError<E::Error>> {
    ...
    pub async fn extend<T: IntoIterator<Item = R> + Send>(
        &self,
        iter: T,
    ) -> Result<Vec<RecordIdKey>, HybridSearchError<E::Error>>
    where
        R: AsRef<Document> + Serialize + DeserializeOwned + 'static,
        K: Sync,
    {
    ...
    pub async fn add_context<D>(&self, context: D) -> Result<Vec<RecordIdKey>, HybridSearchError<E::Error>>
    where
        D: IntoDocuments,
    ...
    pub async fn select(&self, id: impl Into<RecordIdKey>) -> Result<R, HybridSearchError<E::Error>>
    where
    ...
    pub async fn delete(&self, id: impl Into<RecordIdKey>) -> Result<Option<R>, HybridSearchError<E::Error>>
    where
        R: Serialize + DeserializeOwned + 'static,
    {
    ...
    pub async fn select_all(&self) -> Result<Vec<R>, HybridSearchError<E::Error>>
    where
        R: Serialize + DeserializeOwned + 'static,
    {
...
impl<'a, C: Connection, R, M: Embedder, K: Chunker> HybridSearchBuilder<'a, C, R, M, K> {
    ...
    pub async fn run_weighted(self) -> Result<Vec<HybridSearchResult<R>>, HybridSearchError<M::Error>>
    where
    ...
    /// # Errors
    /// Returns `HybridSearchError` if the search fails
    pub async fn run_rrf(self) -> Result<Vec<HybridSearchResult<R>>, HybridSearchError<M::Error>>
    where
        R: Serialize + DeserializeOwned + Clone + AsRef<Document> + Send + Sync,
    {
        let k = self.rrf_k;
