...
impl RemoteOpenAICompatibleModelBuilder<true> {
    /// Build the model.
    pub fn build(self) -> Result<RemoteOpenAICompatibleModel, OpenAIBuilderError> {
        Ok(RemoteOpenAICompatibleModel {
            model: self.config.model.unwrap(),
            base_url: self.config.base_url,
            api_key: match self.config.api_key {
                Some(api_key) => api_key,
                None => std::env::var("OPENAI_API_KEY")
                    .map_err(|_| OpenAIBuilderError::OpenAIApiKeyNotSet)?,
            },
