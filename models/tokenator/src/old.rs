...
impl MergeLayerQueue {
    ...
    pub fn resolve(
        ...
        for regex_match in tokenizer.regex.find_iter(input) {
            tokens.resize(fill_index + regex_match.len(), TokenData::DEFAULT);
            ...
            for resolved in 1..regex_match.len() + 1 {
                ...
                for (i, level) in levels.iter_mut().enumerate() {
                    ...
                    for _ in level.current_index..prev_level_resolved {
                        level.process_token(token_buffer, &mut layers_used, i as u8, tokenizer);
                    }
                    if level.current_index == regex_match.len() {
                        println!("last token");
                        level.finish(token_buffer, &mut layers_used, tokenizer);
                    }
                    if level.resolved_index == prev_resolved {
                        tracing::trace!("update stopped at level: {i}");
                        break;
                    }
                    prev_level_resolved = level.resolved_index;
                    println!("done {:?}", level);
                    println!("resolved: {}", level.resolved_index);
                    println!("current_index: {}", level.current_index);
                }
                println!("outer");
            }
    ...
    fn flush(
        ...
        if len > 1 {
            if let Some(next) = tokens.get(self.current_index) {
                let next = next.token;
                let token = unsafe { tokens.get_unchecked_mut(self.resolved_index - 1) };
                println!("fixing merge for token {:?}", token.token);
                merge_table.get(token.token, next, &mut token.merge);
            }
        }
