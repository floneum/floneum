...
fn test_cast_f32_to_i32() {
    ...
    let b: ConcreteTensor<i32, 1> = a.cast();
...
fn test_cast_i32_to_f64() {
    ...
    let b: ConcreteTensor<f64, 1> = a.cast();
...
fn test_cast_f64_to_f32() {
    let a = ConcreteTensor::<f64, 1>::from_slice([3], &[1.5, 2.5, 3.5]);
    let b: ConcreteTensor<f32, 1> = a.cast();
...
fn test_cast_i32_to_i64() {
    let a = ConcreteTensor::<i32, 1>::from_slice([3], &[100, -200, 300]);
    let b: ConcreteTensor<i64, 1> = a.cast();
...
fn test_cast_u8_to_f32() {
    let a = ConcreteTensor::<u8, 1>::from_slice([4], &[0, 127, 200, 255]);
    let b: ConcreteTensor<f32, 1> = a.cast();
...
fn test_cast_2d_tensor() {
    let a = ConcreteTensor::<f32, 2>::from_slice([2, 2], &[1.1, 2.2, 3.3, 4.4]);
    let b: ConcreteTensor<i32, 2> = a.cast();
...
fn test_cast_same_type() {
    let a = ConcreteTensor::<f32, 1>::from_slice([3], &[1.0, 2.0, 3.0]);
    let b: ConcreteTensor<f32, 1> = a.cast();
...
fn test_cast_large() {
    ...
    let a = ConcreteTensor::<f32, 1>::from_slice([size], &data);
    let b: ConcreteTensor<i32, 1> = a.cast();
