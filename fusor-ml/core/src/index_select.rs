    ...
    fn build_index_kernel(&self, kernel: &mut GenericKernel) {
        ...
        output.check_bounds(
            ...
            |kernel| {
                ...
                indexes.strided_index(kernel, (0..rank).map(|_| format!("merged_index_{}", self.dimension)));
                writeln!(kernel, ";").unwrap();
                writeln!(kernel, "let select_index_value = {indexes}[indices_memory_index];").unwrap();
...
async fn test_reproducing_qwen_issue() {
    ...
    let hpos_indices = Tensor::new(&device, &[0u32, 0, 1, 1, 0, 0, 1, 1, 2, 2, 3, 3, 2, 2, 3, 3]);
...
async fn test_exact_failing_scenario() {
    ...
    let hpos_indices = Tensor::new(&device, &[0u32, 0, 1, 1, 0, 0, 1, 1, 2, 2, 3, 3, 2, 2, 3, 3, 0, 0, 1, 1, 0, 0, 1, 1, 2, 2, 3, 3, 2, 2, 3, 3]);
    ...
    for i in 0..10 {
        ...
        let match_status = if actual_first == expected_first { "✅" } else { "❌" };
        println!("Index {}: idx={}, actual={}, expected={}, {}", i, idx, actual_first, expected_first, match_status);
        
        if i == 2 {
            // This is the failing case from the qwen test
            assert_eq!(actual_first, expected_first, 
                "CRITICAL FAILURE at index {}: expected {}, got {}", i, expected_first, actual_first);
        }
    }
...
async fn test_index_select_exact_make_embeds_pattern() {
    ...
    println!("rotary_dim: {}, inv_freq len: {}", rotary_dim, inv_freq_data.len());
    ...
    for i in 0..indices_data.len() / 2 {
        ...
        let idx2 = i * 2 + 1;
        assert_eq!(
            result_data[idx1], result_data[idx2],
            "CRITICAL: Repeated indices {} and {} should give identical results but got:\nRow 1: {:?}\nRow 2: {:?}",
            indices_data[idx1], indices_data[idx2], 
            &result_data[idx1][0..4], &result_data[idx2][0..4]
        );
    }
...
async fn test_reproduce_exact_qwen_bug() {
    ...
    let pos_ids_data = vec![
        [0u32, 0], [0, 1], [1, 0], [1, 1], [0, 2], [0, 3], [1, 2], [1, 3], [2, 0], [2, 1]
    ];
...
async fn test_manual_stride_simulation() {
    ...
    println!("Manual indices: {:?}", manual_indices.to_vec1().await.unwrap());
    
    let slice_indices = pos_ids.i((.., 0));
    println!("Slice indices: {:?}", slice_indices.to_vec1().await.unwrap());
    ...
    assert_eq!(manual_result, slice_result, "Manual and slice results should be identical");
}
...
async fn test_debug_stride_issue() {
    ...
    println!("Manual slice values: {:?}", manual_slice.to_vec1().await.unwrap());
    ...
    assert_eq!(result1_data, result2_data, "Both slices should give identical results");
}
...
async fn test_debug_memory_layout() {
    ...
    // They should be identical
    assert_eq!(direct_result, slice_result, "Direct and slice results should be identical");
}
...
async fn test_compare_slice_vs_direct_indices() {
    ...
    for i in 0..direct_data.len() {
        if direct_data[i][0] != slice_data[i][0] {
            println!("❌ Mismatch at row {}: direct={}, slice={}", 
    ...
    assert_eq!(direct_data[2][0], slice_data[2][0], 
