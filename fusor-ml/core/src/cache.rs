...
impl<T: FloatDataType> AttentionMask<T> {
    ...
    pub fn apply<const R: usize>(&self, attention_scores: &Tensor<R, T>) -> Tensor<R, T>
    where
        (Tensor<2, T>, Tensor<R, T>): MaxRank<R, T>,
    {
        const {
    ...
    pub fn forward<const R: usize>(&self, attention_scores: &mut Tensor<R, T>)
    where
        (Tensor<2, T>, Tensor<R, T>): MaxRank<R, T>,
    {
