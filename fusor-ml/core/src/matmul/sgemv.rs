...
pub(crate) fn sgemv(
    ...
    let input_datatype = op.datatype;
    ...
    writeln!(kernel, "while (index < {k_size}) {{").unwrap();
    {
        ...
        for i in 0..vector_size {
            ...
            let b_base = format!(
                "select({input_datatype}(0.0), {input_b}[b_start_index + input_b_{i}_index * {b_row_stride} + col_index * {b_col_stride}], input_b_{i}_index < {k_size})"
            );
        ...
        for i in 0..vector_size {
            ...
            let a_base = format!(
                "select({input_datatype}(0.0), {input_a}[a_start_index + {row_index} * {a_row_stride} + input_a_{i}_index * {a_col_stride}], input_a_{i}_index < {k_size} && {row_index} < {m_size})"
            );
